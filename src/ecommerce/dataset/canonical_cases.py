...
class CanonicalCaseGenerator:
    ...
    def _return_cases(self) -> list[CanonicalCase]:
        ...
        cases.append(CanonicalCase(
            ...
            category_hint="return",
            turns=[
                {"role": "user", "content": "充电宝才买两天就充不进电！！！太差了！要退货！！！"},
            ],
            context={
                "days_since_delivery": 2,
                "quality_issue": True,
        ...
        cases.append(CanonicalCase(
            ...
            turns=[
                ...
                {"role": "assistant", "content": "您好，这里是3C数码旗舰店，请问需要什么帮助？"},
    ...
    for case in cases:
        ...
        if case.get("is_behavior") or not case.get("policy_backed", True):
            ...
            if intent not in behavior_intents:
                errors.append(
                    f"{cid}: behavior intent {intent!r} not in known behavior rules"
                )
            else:
        ...
        if match.policy_id:
            ...
            if expected_decision != "reject" and case.get("requires_human") and not match.requires_human:
                errors.append(
                    f"{cid}: requires_human mismatch: expected=True, got=False"
                )
        else:
            # Partial or no match
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate and validate canonical test cases"
    )
    parser.add_argument(
        "--policies",
        type=str,
